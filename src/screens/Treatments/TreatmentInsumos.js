...
const ViewAllTreatments = ({ navigation,route }) => {
  ...
  useEffect(() => {
    db.transaction((tx) => {
      tx.executeSql(`SELECT * FROM TratamientosInsumos`,
        [], (tx, results) => {
  ...
  const listItemView = (item) => {
    return (
      ...
      <View key={item.Id_Tratamiento} style={styles.listItemView}>
        ...
        <MyText text="Id Insumo" style={styles.textNegrita} />
      ...
      <ScrollView>
        <View>
          <View>
            ...
            />
            <MySingleButton style={styles.button}
              title="Agregar Insumo al Tratamiento"
              btnColor="#006400"
